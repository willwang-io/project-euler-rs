...
pub fn continued_fraction_period(n: i32) -> Vec<i32> {
    let mut p = 0;
    let mut q = 1;
    let mut d = vec![];
...
pub fn convergent_reciprocal(i: usize, n: usize, arr: &[i32]) -> (Vec<u8>, Vec<u8>) {
    if i + 1 == n {
        return (to_digit_le(arr[i]), vec![1]);
    }
    let (num, den) = convergent_reciprocal(i + 1, n, arr);
